...
def compute_ece(conf: np.ndarray, correct: np.ndarray, n_bins: int = 5) -> float:
    """Estimate expected calibration error (ECE) by comparing model confidence and empirical accuracy in bins."""
    
    # Create bin edges from 0 to 1, equally spaced
    ...
    for i in range(n_bins):
        # lower and upper bound of the bin
        ...
        if i == n_bins - 1:
            ...
            # For other bins, upper edge is exclusive
            mask = (conf >= lo) & (conf < hi)
        ...
        # Weighted contribution of this bin to overall ECE
        # Weight is proportional to the fraction of total samples in the bin
